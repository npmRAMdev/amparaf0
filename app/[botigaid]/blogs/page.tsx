...
export default async function BlogPage ({
  ...
  return (
    <div className='px-2 py-3'>
      <p className='text-stone-2 font-bold text-xl text-center py-1 sm:py-3'>blog</p>
      {blogs && blogs.map((blog: Blog) => 
        <Link href={`/${botigaid}/blogs/${blog.id}`} key={blog.id} >
          <div className="bg-stone-6 rounded my-2 p-1 space-y-4 flex flex-col max-w-2xl mx-auto">
            <div className='p-2 sm:p-3 space-y-2'>
              ...
              <div className="overflow-hidden rounded-lg flex justify-center">
                <img src={blog.image} alt={blog.title} className="h-28 sm:w-30 sm:h-full object-cover rounded" />
              </div>
